...
Acoording to the types of the prescibed design domain, topology optimization can be classified into discrete (truss/beam) approaches, pioneered by Dorn\cite{Dorn1964}, and continuu (pixel/voxel) approaches, pioneered by Bends{\o}e and Kikuchi \cite{Bendsoee1988}. Acoording to the types of optimization algorithm utilized, topology optimization can be classified into gradient methods and genetic algorithm. Recently, non-gradient methods received serious critique \cite{Sigmund2011} from the topology optimization community regarding its applicability in continuum structure problems. It is due, mainly, to its lack of sensititives and the associated computational inefficiency. However, gradient methods are inapplicable to the problems that have non-differentiable objectives and constraints, for instance, as in case for the manufacturability objectives in the multi-components topology optimization.
...
\subsection*{Material density field}
In a prescibed, fixed design domain $\Omega$, a characterisitic function $\chi$ is defined to descirbe the material domain $\Omega_{d}$ to be optimized:
\begin{equation}
    \chi(\mathbf{x})=
    \begin{cases}
...
\begin{equation}\label{characterisitic function}
    ...
    \end{cases}
\end{equation}
To eliminate checkerboard patterns therefore generating mesh-independent results, the Helmholtz PDE filter is introduced to regularize $\phi$:
...
\begin{equation}\label{density field}
    \rho = \tilde{H}(\tilde{\phi})
\end{equation}
After the series of regularization from $\phi \ \text{to} \ \rho $, the resulting density field is bounded between $0 \ \text{and} \ 1$.
