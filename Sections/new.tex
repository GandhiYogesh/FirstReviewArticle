...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
An objective function $\Phi$ represents the quantity being minimized or maximized to maximize the system's performance—the density of the material in each location as a design variable $\rho$ for the optimization problem. Design space $\Omega$ defines the optimizable volume $V^{*}$ for the design to exist, and the volume in the design space that the optimizer cannot modify is considered non-optimizable volume $\Gamma_s$. The constraints $G_{i}$ are characteristics that the solution must satisfy. The finite element method evaluates the field $\mathbf{U}$ that satisfies a linear or nonlinear state equation since these equations do not have a known analytical solution.
...
The discrete TopOpt uses binary design variables, one for each of the finite elements in the mesh of the structural domain. The design variable $1$ implies the finite element is filled with material, $0$ indicates void. Consider an objective function $\Phi(\rho, \mathbf{U}(\rho))$, constrained by $g_{i}\leq g_{i}^{*}.i\in[1,M]$, with $\rho$ being the design variables, where $N$ number of finite elements in mesh. The discrete variable optimization problem can be formulated as follows:
\begin{equation}
    \begin{aligned}
        \min _{\rho}   & : \Phi(\rho, \mathbf{U}(\rho))                                                         \\
