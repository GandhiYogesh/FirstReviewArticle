...
\begin{tikzpicture}[scale=.9,every node/.style={minimum size=1cm}]
    ...
    \draw[-latex,thick] (6.2,2) node[right]{$\mathsf{feature \ mapping \ on \ analysis \ grid}$}
    to[out=180,in=90] (4,2);

    \draw[-latex,thick](5.8,-.3)node[right]{$\mathsf{simultaneous \ matrix \ TO}$}
    to[out=180,in=90] (3.9,-1);

    \draw[-latex,thick](5.9,5)node[right]{$\mathsf{fiber \ reinforced \ cylindrical \ bars}$}
    ...
    \draw[-latex,thick](6,-4.5)node[right]{$\mathsf{TO \ of \ FRC}$}
    to[out=180,in=90] (2,-5);
